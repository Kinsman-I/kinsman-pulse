...
/**
 * Utilitários de segurança usados em todos os endpoints públicos.
 * Inclui headers, limite de tentativas, proteção de fotos e validação de URL.
 */
/** Envia os cabeçalhos HTTP de defesa aplicáveis à aplicação. */
function security_headers(): void
{
    ...
    header(
        "Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline' https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self'; connect-src 'self' https://challenges.cloudflare.com https://api.asaas.com; frame-src https://challenges.cloudflare.com; frame-ancestors 'self'; base-uri 'self'; object-src 'none'",
    );
    header("Strict-Transport-Security: max-age=31536000; includeSubDomains");
}

function security_rate_hit(
    string $directory,
    string $key,
    int $limit,
    int $window,
...
/** Rejeita POSTs excessivos e aplica limites às ações expostas a abuso. */
function security_request_guard(): void
{
    ...
    try {
        $retry = security_rate_hit(
            APP_ROOT . "/storage/security",
            $action . "|ip|" . ($_SERVER["REMOTE_ADDR"] ?? "unknown"),
            $limit,
            $window,
        );
...
{
    if (!in_array($user["role"] ?? "", ["student", "professional"], true)) {
        return false;
    }
    $owner = $user["role"] === "student" ? "s.user_id = ?" : "p.user_id = ?";
    $statement = $pdo->prepare(
        "SELECT pp.id FROM progress_photos pp JOIN students s ON s.id=pp.student_id JOIN professionals p ON p.id=s.professional_id WHERE pp.image_path=? AND s.tenant_id=? AND p.tenant_id=s.tenant_id AND $owner LIMIT 1",
    );
...
function security_video_url(string $value): ?string
{
    ...
    if (
        ...
        throw new RuntimeException(
            "Informe um endereço HTTP ou HTTPS válido para o vídeo.",
        );
    }
    return $value;
}
