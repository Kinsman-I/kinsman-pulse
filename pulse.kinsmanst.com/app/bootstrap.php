 ...
 * Carrega ambiente, sessão, conexão PDO e funções compartilhadas de
 * autenticação, CSRF, upload, auditoria e isolamento de aluno.
 */

const APP_ROOT = __DIR__ . "/..";

/** Lê o .env local sem expor seus valores ao código público. */
function load_env(string $path): array
{
    if (!is_file($path)) {
        throw new RuntimeException(
            "Arquivo .env nao encontrado. Consulte docs/INSTALACAO-CPANEL.md.",
        );
    }
    $values = [];
    foreach (
...
/** Retorna uma conexão PDO configurada com prepared statements reais. */
function db(): PDO
{
    static $pdo;
...
// -----------------------------------------------------------------------------
// Helpers de URL, sessão, CSRF e mensagens temporárias.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
/** Carrega o usuário ativo e resolve a identidade visual correspondente. */
function current_user(): ?array
{
    ...
    if ($user) {
        ...
        if (
            !isset($_SESSION["auth_fingerprint"]) ||
            !hash_equals($_SESSION["auth_fingerprint"], $fingerprint)
        ) {
            $_SESSION = [];
            $user = false;
        }
...
// -----------------------------------------------------------------------------
// Upload e isolamento de dados de aluno.
// -----------------------------------------------------------------------------
/** Valida e armazena imagens recebidas em uma pasta pública controlada. */
function upload_image(string $field, string $folder): ?string
{
    if (empty($_FILES[$field]["tmp_name"])) {
        return null;
    }
    if (!preg_match("/\A[a-z]+\z/", $folder)) {
    ...
    if ($file["size"] > $max) {
        throw new RuntimeException("A imagem ultrapassa o tamanho permitido.");
    }
...
{
    if ($user["role"] === "admin") {
        return ["$alias.tenant_id = ?", [$user["tenant_id"]]];
    }
    if ($user["role"] === "professional") {
        return [
            "$alias.professional_id = (SELECT id FROM professionals WHERE user_id=?)",
            [$user["id"]],
        ];
    }
