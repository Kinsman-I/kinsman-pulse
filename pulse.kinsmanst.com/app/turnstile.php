...
/** Integração Cloudflare Turnstile para os formulários públicos. */
...
function turnstile_widget(string $action): string
{
    ...
    return '<div class="turnstile-wrap"><div class="cf-turnstile" data-sitekey="' .
        e($siteKey) .
        '" data-action="' .
        e($action) .
...
{
    if (trim((string) env("TURNSTILE_SITE_KEY", "")) === "") {
        return "";
    }
    return '<script src="https://challenges.cloudflare.com/turnstile/v0/api.js" async defer></script>';
}

/** Confirma no servidor se o token corresponde à ação esperada. */
function verify_turnstile(string $expectedAction): void
{
    ...
    if ($siteKey === "" || $secret === "") {
        throw new RuntimeException(
            "A proteção Cloudflare está incompleta. Configure as duas chaves do Turnstile.",
        );
    }
    ...
    if (!is_string($raw) || $raw === "") {
        throw new RuntimeException(
            "Não foi possível validar a segurança agora. Tente novamente.",
        );
    }
    ...
    if (!$valid || !$actionMatches) {
        throw new RuntimeException(
            "A verificação de segurança expirou ou é inválida. Tente novamente.",
        );
    }
}
